import { printTaskTable, loadAllTaskLists } from './utils/utils';
import readline from 'readline';
import chalk from 'chalk';

console.clear();

const taskLists = loadAllTaskLists();
let currentListName = Object.keys(taskLists)[0];
let currentManager = taskLists[currentListName];
const infoMessageColor = chalk.bold.grey;
const warnMessageColor = chalk.bold.bgYellow.black;
...
console.log(chalk.grey('App was started, for command list type "help"'));
...
rl.on('line', (input) => {
  ...
  switch (command) {
    ...
    case 'list':
      console.clear();
      printTaskTable(currentManager.listTasks());
      console.log(
        infoMessageColor('enter command. For command list type "help"'),
      );
      break;
    case 'complete':
      if (args.length === 1) {
    ...
    case 'help':
      ...
      console.log(
        infoMessageColor('todo-lists - list of all available to-do lists'),
      );
      console.log(
        infoMessageColor(
          'switch-list <title> - switches which list is going to be edited',
        ),
      );
      console.log(
        infoMessageColor(
          'add <title> - adds item with <title> into to-do list',
        ),
      );
      console.log(
        infoMessageColor('list - lists items in to-do list currently'),
      );
      console.log(
        infoMessageColor('complete <ID> - completes to-do list item'),
      );
    ...
    default:
      console.log(
        infoMessageColor('Unknown command. For command list type "help"'),
      );
  }
});
